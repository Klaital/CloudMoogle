...
class CharacterAnalysisFormatter
  ...
  def report
    stylesheet = if (CONFIGS[:analysis_reports][:character_analysis][:override_stylesheet_url].nil? || CONFIGS[:analysis_reports][:character_analysis][:override_stylesheet_url].empty?)
      css = File.read(File.join(File.dirname(__FILE__),'..','resources','character_analysis.css'))
      "<style type=\"text/css\">\n#{css}\n</style>\n"
    else
...
<body>
  <div id="header">
    ...
    <span class="datestamp">Generated at: #{Time.now.iso8601}</span>
  </div>
